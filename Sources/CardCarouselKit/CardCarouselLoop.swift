...
enum CardCarouselLoop {
    ...
    static func build(from items: [CardItem]) -> [VirtualSlot] {
        ...
        // Leading buffer: last `bufferSize` items
        for i in (items.count - bufferSize)..<items.count {
        ...
        // Trailing buffer: first `bufferSize` items
    ...
    static func matchingRealSlotIndex(virtualIndex: Int, itemCount: Int) -> Int {
        if virtualIndex < bufferSize {
            // Leading buffer → jump to the real slot
            return bufferSize + (itemCount - bufferSize + virtualIndex)
        } else {
            // Trailing buffer → jump to the real slot
            return bufferSize + (virtualIndex - bufferSize - itemCount)
        }
    }
}
